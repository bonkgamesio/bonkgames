export class BonkCounter {
  ...
  createBackground() {
    ...
    bg.setStrokeStyle(2, 0x444444); // Same style as other counters
    ...
    // Add "BNK" label (shortened from BONK) - positioned right after the digits
    const bonkLabel = this.scene.add.text(
      55, // Position right after the last digit
      0, // Centered vertically
      "BNK", 
      { 
        fontSize: '14px',
        fontFamily: 'Audiowide, monospace',
        fill: '#ff9900', // Match the exact medium ammo color from AmmoDisplay
        stroke: '#000000',
        strokeThickness: 2
      }
    );
    bonkLabel.setOrigin(0, 0.5);
    this.container.add(bonkLabel);
  }
  ...
  createDecimalPoints() {
    ...
    const decimalCircle = this.scene.add.circle(
      ...
      0xff9900 // Match the exact medium ammo color from AmmoDisplay
    );
  ...
  setActiveSegments(displayIndex, value) {
    ...
    // Orange/yellow color for BONK counter - matching AmmoDisplay warning state
    const activeColor = 0xff9900; // Exact match to AmmoDisplay's medium ammo color
