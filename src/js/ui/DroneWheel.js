...
export class DroneWheel {
  ...
  confirmSelection() {
    ...
    if (upgrade.type === 'speed') {
      ...
      // Check if we're in tutorial mode or if prompt not initialized
      if (this.scene.scene.key === 'TutorialScene' || !this.rhythmGame) {
        console.log("In tutorial mode or rhythmGame not initialized, showing message");
        this.scene.events.emit('showFloatingText', {
          x: this.scene.playerManager.player.x,
          y: this.scene.playerManager.player.y - 50,
          text: "FEATURE LOCKED",
      ...
      // Preserve ATM flag and time scale for hack
      ...
      // Start the BONK withdrawal rhythm hack
